@extends('admin.layouts.app')
    @section('meta')
     <title>Laporan Data Jadwal| Sistem Absensi</title>
    ...
    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">Laporan Data Jadwal
        ...
        <div class="row">
            <div class="box box-success">
                <div class="box-body reportstable">
                    <form action="{{ url('admin/export/jadwal') }}" method="post" accept-charset="utf-8" class="ui small form form-filter" id="filterform">
                        {{ csrf_field() }}
                        ...
                        <tbody>
                            ...
                            @foreach ($empSched as $v)
                                <tr>
                                    ...
                                    <td>
                                        @php 
                                        $datefrom = date("l, F j, Y", strtotime($v->date_from));
                                        @endphp 
                                        {{ $datefrom }}
                                    </td>
                                    <td>
                                        @php 
                                        $dateto = date("l, F j, Y", strtotime($v->date_to));
                                        @endphp 
                                        {{ $dateto }}
                                    </td>
                                    </td>
    ...
    $('#btnfilter').click(function(event) {
        ...
        $.ajax({
            url: '{{url('admin/laporan/getjadwal')}}',type: 'get',dataType: 'json',data: {id: emp_id},headers: { 'X-CSRF-Token': $('meta[name="csrf-token"]').attr('content') },
            ...
            success: function(response) {
                ...
                function showdata(jsonresponse) {
                    ...
                    for (var i = 0; i < employee.length; i++) {
                        var datefrom = employee[i].date_from;
                        var dateto = employee[i].date_to;
                        ...
                        tbody.append("<tr>" + 
                                            ...
                                            "<td>"+employee[i].in_time+"</td>" + 
                                            "<td>"+employee[i].out_time+"</td>" + 
